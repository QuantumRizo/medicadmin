...
import {
    Users, Crown,
    Shield, Loader2, RefreshCw, ChevronUp,
    ChevronDown, Stethoscope, LogOut,
    AlertTriangle, Clock, CheckCircle2, HardDrive
} from 'lucide-react';
...
export const SuperAdminContent = () => {
    ...
    const totalStorageBytes = doctors.reduce((s, d) => s + (d.storage_used_bytes || 0), 0);
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {[
                    { label: 'Doctores Totales', value: totalDoctors, icon: Users, color: 'text-[#1c334a]', bg: 'bg-[#1c334a]/10' },
                    ...
                    { label: 'Almacenamiento Total', value: formatBytes(totalStorageBytes), icon: HardDrive, color: 'text-[#1c334a]', bg: 'bg-[#1c334a]/10' },
                ].map(kpi => (
            ...
            {loading ? (
                ...
                <div className="space-y-3">
                    ...
                    <div className="hidden lg:grid grid-cols-12 gap-3 px-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        <button className="col-span-4 text-left group flex items-center" onClick={() => toggleSort('full_name')}>
                            Doctor / Clínica <SortIcon col="full_name" />
                        </button>
                        <button className="col-span-2 text-center group" onClick={() => toggleSort('plan_name')}>
                            ...
                            >
                                <div className="lg:grid lg:grid-cols-12 lg:gap-3 lg:items-center space-y-4 lg:space-y-0">
                                    ...
                                    <div className="lg:col-span-2 flex lg:justify-center">
                                        <span className={`inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider border ${
                                            isPro
                                                ? 'bg-[#1c334a] text-white border-[#1c334a]'
                                                : 'bg-slate-100 border-slate-200 text-slate-600'
                                        }`}>
                                            {isPro ? <Crown className="w-3.5 h-3.5 text-amber-300" /> : <Shield className="w-3.5 h-3.5 text-slate-400" />}
                                            {doc.plan_name}
                                        </span>
                                    </div>
                                    ...
                                    <div className="lg:col-span-2 lg:text-center">
                                        {isPro ? (
                                            ...
                                            <div className={`flex items-center gap-1.5 text-xs font-bold lg:justify-center ${daysLeft <= 5 ? 'text-amber-600' : 'text-slate-600'}`}>
                                                <Clock className="w-4 h-4" /> {daysLeft} días restantes
                                            </div>
